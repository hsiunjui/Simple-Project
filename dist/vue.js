(function (global, factory) {
  ...
  var Watcher = /*#__PURE__*/function () {
    ...
    function Watcher(vm, exprOrfn, cb, options) {
      ...
      this.cb = cb;
      this.options = options; // 2. 每一组件只有一个watcher 他是为标识e

      this.id = id++;
      this.user = !!options.user; // 3.判断表达式是不是一个函数
    ...
    _createClass(Watcher, [{
      ...
      value: function run() {
        ...
        this.value = value; //执行 hendler (cb) 这个用户wathcer
  ...
  function stateMixin(vm) {
    console.log(vm, 6666); //列队 :1就是vue自己的nextTick  2用户自己的
  ...
  function patch(oldVnode, Vnode) {
    ...
    if (oldVnode.nodeType === 1) {
      ...
      //  diff
      console.log(oldVnode, Vnode); //1 元素不是一样 
      ...
      updataRpors(Vnode, oldVnode.data); //diff子元素 <div>1</div>  <div></div>
      ...
      if (oldChildren.length > 0 && newChildren.length > 0) {
        //老的有儿子 新有儿子
        //创建方法
        updataChild(oldChildren, ewChildren);
        ...
        for (var i = 0; i < newChildren.length; i++) {
          var child = newChildren[i]; //添加到真实DOM
  ...
  function updataChild(oldChildren, ewChildren, el) {} //添加属性
  ...
  var vnode2 = render2.call(vm2); //patch 比对
